...
onAuthStateChanged(auth, async user => {
  ...
  try {
    ...
    if (adminAsTeacher) {
      // Admin must also have "teacher" in their role array to proceed
      ...
      // Show the admin-mode banner at the top of the teacher dashboard
    ...
    // Applies to both regular teachers and admin-as-teacher.
    ...
    await loadTeacherProfile(user, data);
...
async function loadTeacherProfile(user, data) {
  try {
    const roles = resolveRole(data.role);

    // Show admin switcher if user also has admin role
    if (hasRole(roles, "admin")) {
      document.getElementById("adminSwitchBtn").classList.remove("hidden");
    }
